...
router.post('/', upload.single('image'), function (request, response) {
    ...
    var postId = random(60);

    fs = require('fs');

    checkProfilePic(userId);
    ...
    MongoClient.connect(url, function (err, db) {
        ...
        dbo.collection(userId + '_posts').insertOne(post, function (err) {
            if (err) throw err;
        });
        ...
        decode_save_base64(encoded_image, fileName + '.jpg', userId);
